...
var _mod_lookup: Dictionary = {}    # "type_strN" → component dict
var _effect_lookup: Dictionary = {} # effect_value int → component dict
...
func _on_spell_creation_spell_data_created(spell_array: Array) -> void:
	...
	for component in _flat_spell_array:
		match component["type"]:
			"mod_float", "mod_int", "mod_vec", "mod_bool":
				_mod_lookup[component["type"] + str(int(component["value"]))] = component
			"effect":
				_effect_lookup[int(component["value"])] = component
			"casting":
				if current_casting_type == -1:
...
func _apply_self_effects(is_toggle: bool) -> void:
	for component in _flat_spell_array:
		...
		if is_toggle:
			...
			player_root.add_child(_factory.make_effect_node(ev, component, SpellGlobals.DEFAULT_EFFECT_DURATION * _get_duration_mult()))
...
func _find_mod(type_str: String, value: int) -> Dictionary:
	return _mod_lookup.get(type_str + str(value), {})

func _find_effect_component(effect_value: int) -> Dictionary:
	return _effect_lookup.get(effect_value, {})
