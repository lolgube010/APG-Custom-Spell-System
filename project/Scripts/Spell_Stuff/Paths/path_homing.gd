...
func _ready() -> void:
	super()  # sets parent_spell

func _physics_process(delta: float) -> void:
	if not is_instance_valid(parent_spell):
		return

	# Lazy init: global_transform is valid only after the spell enters the scene tree.
	if not _initialized:
		_direction = -parent_spell.global_transform.basis.z
		_initialized = true

	# Acquire / re-acquire target: on first frame, when timer expires, or when target dies.
